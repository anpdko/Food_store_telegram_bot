...
class Order(models.Model):
    ...
    readyStatus = models.ForeignKey(Status, verbose_name = "Статус", on_delete = models.PROTECT, blank=True, null=True)
    price = models.DecimalField(max_digits = 10, decimal_places = 2, verbose_name = "Цена(грн.)", blank=True, null=True)
    created = models.DateTimeField(verbose_name='Дата создания', auto_now_add=True, blank=True, null=True)
    updated = models.DateTimeField(verbose_name='Дата изменения', auto_now=True, blank=True, null=True)
    paid = models.BooleanField(verbose_name='Оплата', blank=True, null=True)
    IdTelegram =  models.CharField(verbose_name='Id телеграмм', max_length=50, blank=True, null=True)
